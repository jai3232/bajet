...
$sql = "SELECT * FROM unjuran WHERE ((os = '29000' OR os = 'AMANAH' OR os LIKE '4%' OR os LIKE 'P%' OR os LIKE 'B%')  AND sah=1 AND tahun='$currentYear' AND kod='A' AND (id_jabatan=$id_jabatan OR FIND_IN_SET('$id_jabatan', unjuran.kongsi) ))";
...
$this->registerJs('
	...
	$("#myTable tr").on("click", function(){
		...
		$("#penceramah-kod_unjuran").val(td.eq(1).html());
		$("#penceramah-bahagian").val(td.eq(6).children().attr("id"));
		...
		$("#penceramah-id_jabatan").val(td.eq(6).children().prop("id"));
		$("#unjuran_info").slideDown(150);
		$(".penceramah-form").slideDown(200);
