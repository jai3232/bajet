...
<div class="finance-form">
    ...
    <table id="kodA" class="table table-striped table-bordered table-hover table-responsive list-view">
    	<tbody>
            ...
            <tr><th>Jumlah Tuntutan</th><td>RM <span id="jumlah_tuntutan"><?= number_format($model->jumlah_OT, 2) ?></span></td></tr>

            <tr><th>Kelulusan </th><td>
                                    <input name="Ot[lulus]" type="radio" value="0" id="lulus"> Lulus tuntutan dengan jumlah yang sama. <br>
                                    <input name="Ot[lulus]" type="radio" value="1" id="tukar"> Lulus dengan perubahan jumlah. <br>
                                    <input name="Ot[lulus]" type="radio" value="2" id="tolak"> Tolak. <br>
                                   </td>
            </tr>
    ...
    <?= $form->field($model, 'jumlah_OT', ['inputOptions' => ['value' => Yii::$app->formatter->asDecimal($model->jumlah_OT), 'class' => 'form-control']])->textInput(['readonly' => true, 'type' => 'number'])->label('Jumlah Tuntutan (RM)') ?>
...
$(":radio").click(function(){
    if($(this).val()/1 == 0) {
        $("#ot-status").val('B');
        $("#ot-jumlah_ot").prop("readonly", true);
        $("#ot-jumlah_ot").val($("#jumlah_tuntutan").text().replace(/,/g, ""));

    }
    if($(this).val()/1 == 1){
        $("#ot-jumlah_ot").prop("readonly", false);
        $("#ot-status").val('B');
    }
