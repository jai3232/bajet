...
$this->title = 'Laporan Baki Perbelanjaan Bahagian '.$year;
		...
		<table id="belanja-os-jabatan" class="table table-striped table-hover table-condensed table-bordered">
			<thead>
				<tr>
					...
					?>
					<th class="text-center">Jumlah Baki</th>
				</tr>
				<tr>
					<th>No</th><th>OS</th>
					<?php
			...
			<?php
				...
				$baki_jabatan = array_fill(0, count($id_jabatan), 0);
			...
			?>
				<tr>
					...
					?>
						<td class="text-right">
							<?php
								...
								$baki = $agihan_jabatan - $belanja;
								$baki_os += $baki;
								$belanja_jabatan[$key_jabatan] += $belanja;
								$baki_jabatan[$key_jabatan] += $baki;
								$percent = ($agihan_jabatan == 0) ? 0 : $baki / $agihan_jabatan;
					...
					<?php
						...
						$percent_os = $jumlah_agihan == 0 ? 0 : $baki_os / $jumlah_agihan;
					?>	
					<td class="text-right"><?= number_format($baki_os, 2).' / '.number_format($jumlah_agihan, 2).' ('.number_format($percent_os * 100, 1).')' ?></td>
				</tr>	
			<?php
				...
				<tr>
					...
					<?php
						...
						foreach ($id_jabatan as $key_jabatan => $value_id_jabatan) {
							...
							$percent_jabatan = $jumlah_agihan_jabatan == 0 ? 0 : $baki_jabatan[$key_jabatan] / $jumlah_agihan_jabatan;
							$total_baki += $baki_jabatan[$key_jabatan];
							echo '<th class="text-right">'.number_format($baki_jabatan[$key_jabatan], 2)
								.' / '.number_format($jumlah_agihan_jabatan, 2)
								.' ('.number_format($percent_jabatan * 100, 1).')</th>';
						}
						$total_percent = $jumlah_permohonan_semua == 0 ? 0 : $total_baki / $jumlah_permohonan_semua;
					?>
					<th class="text-right"><?= number_format($total_baki, 2).' / '.number_format($jumlah_permohonan_semua, 2).' ('.number_format($total_percent * 100, 1).')' ?></th>
				</tr>
			</tfoot>
		</table>
